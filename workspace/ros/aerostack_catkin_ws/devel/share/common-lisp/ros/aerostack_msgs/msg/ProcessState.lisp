...
(cl:defmethod cl:initialize-instance :after ((m <ProcessState>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'ProcessState)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name aerostack_msgs-msg:<ProcessState> is deprecated: use aerostack_msgs-msg:ProcessState instead.")))

(cl:ensure-generic-function 'state-val :lambda-list '(m))
(cl:defmethod state-val ((m <ProcessState>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader aerostack_msgs-msg:state-val is deprecated.  Use aerostack_msgs-msg:state instead.")
  (state m))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<ProcessState>)))
  ...
  (cl:format cl:nil "#Current state of a process enum~%~%uint8 state  # State of the process~%~%uint8 Created=1~%uint8 ReadyToStart=2~%uint8 Running=3~%uint8 Paused=4~%uint8 Started=7~%uint8 NotStarted=8~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'ProcessState)))
  "Returns full string definition for message of type 'ProcessState"
  (cl:format cl:nil "#Current state of a process enum~%~%uint8 state  # State of the process~%~%uint8 Created=1~%uint8 ReadyToStart=2~%uint8 Running=3~%uint8 Paused=4~%uint8 Started=7~%uint8 NotStarted=8~%~%~%"))
