...
class dronePositionTrajectoryRefCommand {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.droneTrajectory.forEach((val) => {
      bufferOffset = dronePositionRefCommand.serialize(val, buffer, bufferOffset);
    });
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new dronePositionTrajectoryRefCommand(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.droneTrajectory[i] = dronePositionRefCommand.deserialize(buffer, bufferOffset)
    }
  ...
  static getMessageSize(object) {
    ...
    length += 12 * object.droneTrajectory.length;
    return length + 9;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new dronePositionTrajectoryRefCommand(null);
    ...
    if (msg.droneTrajectory !== undefined) {
      ...
      for (let i = 0; i < resolved.droneTrajectory.length; ++i) {
        resolved.droneTrajectory[i] = dronePositionRefCommand.Resolve(msg.droneTrajectory[i]);
      }
    }
    else {
...
module.exports = dronePositionTrajectoryRefCommand;
