...
class VectorROIs {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.obs[i] = Target.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new VectorROIs(null);
    ...
    if (msg.obs !== undefined) {
      ...
      for (let i = 0; i < resolved.obs.length; ++i) {
        resolved.obs[i] = Target.Resolve(msg.obs[i]);
      }
    }
    else {
      resolved.obs = []
